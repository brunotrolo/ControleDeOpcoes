...
function onOpen(e) {
  try {
    ...
    ui.createMenu('⚙️ Automação')
        ...
        .addSeparator()
        .addItem('📡 6. Escanear Opções Oportunidades (API OPLab)', 'AtualizarScanner_Menu')
        .addSeparator()
        .addItem('📊 7. Séries de Opções (API OPLab)', 'SyncSeriesInstrumento_Menu')
        .addItem('💰 8. Melhores Taxas de Lucro (API OPLab)', 'SyncBestCoveredOptionsRates_Menu')
        .addItem('📈 9. Maiores Volumes em Opções (API OPLab)', 'SyncHighestOptionsVolume_Menu')
        .addItem('⚡ 10. Maiores Variações em Opções (API OPLab)', 'SyncHighestOptionsVariation_Menu')
