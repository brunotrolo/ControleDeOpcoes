/**
 * @fileoverview 021_SyncCorrelIbovRanking.gs - v1.0
 * =====================================================
 * ENDPOINT: GET /market/statistics/ranking/correl_ibov
 * ABA: RANKING_CORREL_IBOV
 * Ativos ranqueados pela correlacao com o indice IBOV.
 *
 * RESPOSTA (8 campos flat):
 *   symbol, updated_at, cnpj, attribute (valor da correlacao),
 *   attribute_name, short_name, name, sector
 *
 * ESTRATEGIA: Busca dois rounds (maior correlacao = bull mkt,
 * menor correlacao = descorrelacionados/hedge) com deduplicacao.
 * =====================================================
 */

const CORREL_024_SHEET         = 'RANKING_CORREL_IBOV';
const CORREL_024_LIMIT         = 200;
const CORREL_024_FINANCIAL_MIN = 1000000; // R$ 1M/dia minimo de liquidez
const CORREL_024_DAYS          = 30;

const CORREL_024_HEADERS = [
...
function SyncCorrelIbovRanking_Menu() {
  _menuBridge('Ranking Correlacao IBOV (OPLab)', orquestrarSyncCorrelIbov);
}
...
function orquestrarSyncCorrelIbov() {
  ...
  SysLogger.log('CorrelIbov', 'START',
    ...
    JSON.stringify({
      aba: CORREL_024_SHEET, limit: CORREL_024_LIMIT,
      financial_volume_min: CORREL_024_FINANCIAL_MIN,
      days: CORREL_024_DAYS,
      timestamp: new Date().toISOString()
    })
  );

  const ss    = SpreadsheetApp.getActiveSpreadsheet();
  const sheet = _garantirAba024(ss);
  _limparAba024(sheet);
  SysLogger.log('CorrelIbov', 'INFO',
    'Aba "' + CORREL_024_SHEET + '" pronta (' + CORREL_024_HEADERS.length + ' colunas).'
  );
  ...
  [
    ...
    var url   = OplabService._baseUrl +
      ...
      '&limit=' + CORREL_024_LIMIT +
      '&financial_volume_start=' + CORREL_024_FINANCIAL_MIN +
      '&days=' + CORREL_024_DAYS;
    ...
    try {
      ...
      // Filtro: apenas acoes brasileiras puras (reutiliza _isAcaoBrasileira do 018)
      ...
      if (linhasFiltradas.length > 0) {
        sheet.getRange(proximaLinha, 1, linhasFiltradas.length, CORREL_024_HEADERS.length)
...
function _garantirAba024(ss) {
  var sheet = ss.getSheetByName(CORREL_024_SHEET);
  if (!sheet) sheet = ss.insertSheet(CORREL_024_SHEET);
  sheet.getRange(1, 1, 1, CORREL_024_HEADERS.length).setValues([CORREL_024_HEADERS]);
  return sheet;
}

function _limparAba024(sheet) {
  var lr = sheet.getLastRow();
  if (lr > 1) sheet.getRange(2, 1, lr - 1, sheet.getLastColumn()).clearContent();
}

function testCorrelIbovRanking() {
  console.log('=== HOMOLOGACAO 024_SyncCorrelIbovRanking ===');
  var url = OplabService._baseUrl +
