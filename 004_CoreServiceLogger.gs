...
const DataExtractorService = {
  ...
  /**
   * Extrai todos os tickers únicos da aba DADOS_ATIVOS.
   * Usado pelo 014_SyncGetInstrumentsSeries para buscar séries de TODOS
   * os ativos monitorados — não apenas os que têm posição aberta no COCKPIT.
   * @returns {string[]} Array de tickers em UPPER_CASE, sem duplicatas, ordenados.
   */
  extractTodosAtivos() {
    SysLogger.log("Extractor", "INFO",
      `Lendo tickers de DADOS_ATIVOS (${SYS_CONFIG.SHEETS.ASSETS}).`
    );

    try {
      const sheet = SpreadsheetApp.getActiveSpreadsheet()
        .getSheetByName(SYS_CONFIG.SHEETS.ASSETS);

      if (!sheet) throw new Error(`Aba "${SYS_CONFIG.SHEETS.ASSETS}" não encontrada.`);

      const lastRow = sheet.getLastRow();
      if (lastRow < 2) {
        SysLogger.log("Extractor", "AVISO", "DADOS_ATIVOS está vazia.");
        return [];
      }
      ...
      if (tickerCol === -1) throw new Error('Coluna "TICKER" não encontrada em DADOS_ATIVOS.');

      const valores = sheet.getRange(2, tickerCol + 1, lastRow - 1, 1).getValues();

      const tickers = [
        ...new Set(
          valores
            .map(row => Sanitizador.textoPuro(row[0]))
            .filter(t => t && t.length > 0)
        )
      ].sort();

      SysLogger.log("Extractor", "SUCESSO",
        `${tickers.length} ticker(s) únicos encontrados em DADOS_ATIVOS.`,
      ...
      SysLogger.log("Extractor", "CRITICO",
        "Falha ao extrair tickers de DADOS_ATIVOS.", String(e.message)
      );
      return [];
    }
  }
