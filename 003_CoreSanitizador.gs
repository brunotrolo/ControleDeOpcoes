...
const Sanitizador = {
  ...
  /**
   * Extrai apenas a parte da data (yyyy-mm-dd) de qualquer valor ISO ou Date.
   * Usado para colunas EXPIRY vindas da OPLab — evita que o Sheets exiba hora e timezone.
   * Exemplos:
   *   "2026-05-15T03:00:00.000Z" → "2026-05-15"
  ...
  dataSoData(valor) {
    ...
    // Se já é string ISO ou date-only, extrai os primeiros 10 chars (yyyy-mm-dd)
    if (typeof valor === 'string') {
      const s = valor.trim();
      if (/^\d{4}-\d{2}-\d{2}/.test(s)) return s.substring(0, 10);
    }

    // Se é objeto Date, formata manualmente para evitar problema de timezone
