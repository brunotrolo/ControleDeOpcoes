/**
 * @fileoverview 018_SyncM9M21Ranking.gs - v1.0
 * ═══════════════════════════════════════════════════════════════
...
const M9M21_018_SHEET          = 'RANKING_TENDENCIA_M9M21';
const M9M21_018_LIMIT          = 200;
const M9M21_018_FINANCIAL_MIN  = 1000000; // R$ 1M/dia — filtra ativos com liquidez relevante
const M9M21_018_DAYS           = 30;      // Só ativos atualizados nos últimos 30 dias
...
function orquestrarSyncM9M21() {
  ...
  SysLogger.log('M9M21Ranking', 'START',
    ...
    JSON.stringify({
      aba: M9M21_018_SHEET, limit: M9M21_018_LIMIT,
      financial_volume_min: M9M21_018_FINANCIAL_MIN,
      days: M9M21_018_DAYS,
  ...
  const sheet = _garantirAba018(ss);
  SysLogger.log('M9M21Ranking', 'INFO', `Aba "${M9M21_018_SHEET}" pronta (${M9M21_018_HEADERS.length} colunas).`);
  ...
  [
    ...
    const url   = `${OplabService._baseUrl}/market/statistics/ranking/m9_m21?sort=${sort}&limit=${M9M21_018_LIMIT}&financial_volume_start=${M9M21_018_FINANCIAL_MIN}&days=${M9M21_018_DAYS}`;
    SysLogger.log('M9M21Ranking', 'INFO',
      `API call: GET /market/statistics/ranking/m9_m21?sort=${sort}&limit=${M9M21_018_LIMIT}&financial_volume_start=${M9M21_018_FINANCIAL_MIN}&days=${M9M21_018_DAYS} [tendência de ${label}]`
    );
    ...
    try {
      ...
      if (linhasFiltradas.length > 0) {
        sheet.getRange(proximaLinha, 1, linhasFiltradas.length, M9M21_018_HEADERS.length).setValues(linhasFiltradas);
...
function _garantirAba018(ss) {
  let sheet = ss.getSheetByName(M9M21_018_SHEET);
  if (!sheet) { sheet = ss.insertSheet(M9M21_018_SHEET); }
  sheet.getRange(1, 1, 1, M9M21_018_HEADERS.length).setValues([M9M21_018_HEADERS]);
  return sheet;
}
function _limparAba018(sheet) {
  const lr = sheet.getLastRow();
  if (lr > 1) sheet.getRange(2, 1, lr - 1, sheet.getLastColumn()).clearContent();
}

function testM9M21Ranking() {
  console.log('=== HOMOLOGAÇÃO 018_SyncM9M21Ranking ===');
  const url  = `${OplabService._baseUrl}/market/statistics/ranking/m9_m21?sort=desc&limit=3&financial_volume_start=${M9M21_018_FINANCIAL_MIN}&days=${M9M21_018_DAYS}`;
  const data = ApiClient._fetchData(url, { headers: OplabService._getHeaders() });
