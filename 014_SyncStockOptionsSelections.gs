/**
 * @fileoverview 101_CoreScannerOptions - v6.5 (Greeks Pipeline Edition)
...
const CoreScannerOptions = {
  ...
  run() {
    ...
    try {
      ...
      tickers.forEach((ticker, index) => {
        ...
        // SysLogger.log(this._serviceName, "INFO", `API Request: Buscando opções para ${ticker}`);

        const opcoesAPI = OplabService.getOptionsByTicker(ticker);

        if (!opcoesAPI || !Array.isArray(opcoesAPI) || opcoesAPI.length === 0) {
        // 🚀 LOG DE AUDITORIA: Response Vazio/Falha  
        // SysLogger.log(this._serviceName, "WARN", `API Response VAZIO/ERRO: Nenhuma opção para ${ticker}`);
          stats.erros++;
          return;
        }

        // 🚀 LOG DE AUDITORIA: Response Sucesso (Com Payload Completo)
        // SysLogger.log(this._serviceName, "INFO", `API Response OK: ${ticker} (${opcoesAPI.length} opções encontradas)`, JSON.stringify(opcoesAPI));
        ...
        selecionadas.forEach(op => {
          // Passando a Selic e a IV Fallback do Ativo Pai para a função de mapeamento
  ...
  _mapearParaDUD(ticker, op, spotReal, headers, selic, fallbackIV) {
    ...
    let ivCalc = fallbackIV || 0.35; // Usa o IV da aba Ativos ou um padrão de 35%
    ...
    try {
      if (midPrice > 0.01 || !fallbackIV) {
        if (midPrice <= 0) midPrice = 0.05; // Evita Crash Matemático no Newton-Raphson
    ...
    // 🛡️ O MAPA DEFINITIVO DE TIPAGEM + GREGAS
    const map = {
      ...
      "EXCH_TIME": (() => {
          ...
          if (val > 946684800000) return new Date(val); // Converte o Timestamp da OpLab para Data/Hora
      ...
      // DADOS INJETADOS EM TEMPO DE EXECUÇÃO
